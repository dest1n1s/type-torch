...
const reservedTsVariableNames = ['var']
...
function getTsArgName(arg: Arg) {
  return reservedTsVariableNames.includes(arg.arg_name) ? `${arg.arg_name}_` : arg.arg_name
}
...
function getExtractedCTypeArg({ arg_name, arg_type }: Arg, index: number) {
  switch (arg_type) {
    ...
    case 'TensorOptions':
      return [
        `Napi::Object ${arg_name}_obj__ = info__[${index}].As<Napi::Object>();`,
        `int ${arg_name}_device = ${arg_name}_obj__.Get("device").ToNumber().Int32Value();`,
        `int ${arg_name}_kind = ${arg_name}_obj__.Get("dtype").ToNumber().Int32Value();`
      ]
    case 'String':
      return [
...
function getTsReturn(func: Func) {
  switch (func.returns.type) {
    case 'nothing':
      return 'void'
    case 'dynamic':
      return 'CTensor[]'
    case 'fixed':
      return func.returns.length === 1 ? 'CTensor' : 'CTensor[]'
    case 'bool':
      return 'boolean'
    case 'int64_t':
    case 'double':
...
function getTsArgsList(func: Func) {
  return func.args.map(getTsArgName).join(', ')
}

function getTsTypeArgsList(func: Func) {
  const argsType = func.args
    .map((arg) => {
      const arg_name = getTsArgName(arg)
      const arg_type = arg.arg_type
      switch (arg_type) {
        case 'Scalar':
          return `${arg_name}: CScalar`
        case 'ScalarType':
        case 'Device':
        case 'Layout':
        case 'MemoryFormat':
        ...
        case 'IntListOption':
          return `${arg_name}?: number[]`
        case 'TensorOptions':
          return `${arg_name}: { device: number, dtype: number }`
        case 'Int64Option':
        case 'DoubleOption':
          return `${arg_name}?: number`
        case 'Bool':
    ...
    .join('; ')
  if (argsType === '') {
    return '_: Record<string, never>'
  }
  return `{ ${getTsArgsList(func)} }: { ${argsType} }`
}
...
function writeCpp(funcs: Record<string, Func>, filename: string, directory: string) {
...
function writeTs(funcs: Record<string, Func>, filename: string, directory: string) {
  const tsFile = `${directory}/${filename}.ts`
  fs.writeFileSync(tsFile, '')
  const p = (str: string) => fs.appendFileSync(tsFile, str + '\n')
  p('// THIS FILE IS AUTOMATICALLY GENERATED, DO NOT EDIT BY HAND!')
  p('')
  p('import { CTensor, CScalar, addon } from "./torch_api.js"')
  p('')
  Object.entries(funcs).forEach(([exportedName, func]) => {
    p(`export const atg_${exportedName} = (${getTsTypeArgsList(func)}): ${getTsReturn(func)} => {`)
    p(`  return addon.atg_${exportedName}(${func.args.map(getTsArgName).join(', ')})`)
    p('}')
    p('')
  })
}
...
function main() {
  ...
  writeTs(funcMap, 'torch_api_generated', 'src/wrapper')
}
